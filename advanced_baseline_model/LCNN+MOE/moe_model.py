...
class Classic_MOE(nn.Module):
    ...
    def forward(self, x):
        ...
        # Each expert(x) outputs raw logits shape: [batch_size, 2]
        expert_outputs = [expert(x) for expert in self.experts]

        # Stack along the final dimension -> Shape: [batch_size, 2, num_experts]
        expert_outputs = torch.stack(expert_outputs, dim=-1)

        # Reshapes from [batch_size, freq, time] to [batch_size, freq * time]
        x = x.view(x.size(0), -1)

        # Compute gating weights and apply softmax -> Shape: [batch_size, num_experts]
