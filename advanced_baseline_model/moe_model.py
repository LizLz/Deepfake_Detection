...
class Classic_MOE(nn.Module):
    def __init__(self, experts, feature_shape):
        super(Classic_MOE, self).__init__()

        self.experts = nn.ModuleList(experts)
        input_dim = feature_shape[0] * feature_shape[1]
        self.gating_network = nn.Linear(input_dim, len(experts))

    def forward(self, x):
        expert_outputs = [expert(x).squeeze(1) for expert in self.experts]
        expert_outputs = torch.stack(expert_outputs, dim=-1)

        gating_input = x.view(x.size(0),-1) # if too large, consider GlobalAveragePooling
        gating_weights = F.softmax(self.gating_network(gating_input), dim=-1)
        weighted_output = torch.sum(gating_weights * expert_outputs, dim=-1)

        return weighted_output, gating_weights
        
# - ------------------------- Enhanced MOE with feature transformation and learnable pooling ------------------ -
